...
class KlavisAPI:
    ...
    def get_user_instances(self, user_id: str, platform_name: str) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dictionary containing:
                - instances: List of ExtendedMcpServer objects with server information
                  Each instance includes:
                  - id: Server instance ID
                  - name: Server name
                  - description: Server description (or null)
                  - tools: List of available tools (or null)
                  - authNeeded: Whether authentication is required (default: true)
                  - isAuthenticated: Whether the instance is authenticated (default: false)
        ...
        """
        if not user_id or len(user_id.strip()) < 1:
            raise ValueError("user_id must have minimum length of 1")
        
        if not platform_name or len(platform_name.strip()) < 1:
            raise ValueError("platform_name must have minimum length of 1")
        ...
        result = self._make_request("GET", "/user/instances", params=params)
        logger.info(f"Retrieved user instances for user_id '{user_id}' and platform '{platform_name}': {len(result.get('instances', []))} instances found")
        return result
