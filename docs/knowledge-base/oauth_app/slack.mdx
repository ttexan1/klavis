...
Klavis Slack MCP Server uses the following OAuth scopes:
    ...
    **User Scopes:** `channels:history, channels:read, channels:write.invites, chat:write, groups:history, groups:read, groups:write.invites, im:history, im:read, im:write.invites, mpim:history, mpim:read, mpim:write.invites, search:read, users:read`
</Note>
