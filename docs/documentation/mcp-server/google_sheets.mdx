    ...
    <Steps>
      ...
      <Step title="Create a server instance">
        <CodeGroup>
          ...
          // Create a Google Sheets MCP server instance
          const googleSheetsServer = await klavis.mcpServer.createServerInstance({
        ...
        <Info>
          **Response Information**: The API returns:
          - `serverUrl`: The URL for connecting your MCP client to Google Sheets
          - `instanceId`: Unique identifier for your server instance
          - `oauthUrl`: OAuth authorization URL for Google Sheets authentication
        </Info>
      </Step>
