    ...
    <Steps>
      ...
      <Step title="Create a server instance">
        <CodeGroup>
          ...
          // Create a GitHub MCP server instance
          const githubServer = await klavis.mcpServer.createServerInstance({
        ...
        <Info>
          **Response Information**: The API returns:
          - `serverUrl`: The URL for connecting your MCP client to GitHub
          - `instanceId`: Unique identifier for your server instance
          - `oauthUrl`: OAuth authorization URL for GitHub authentication
        </Info>
      </Step>
