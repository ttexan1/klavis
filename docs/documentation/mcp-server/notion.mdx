    ...
    <Steps>
      ...
      <Step title="Create a server instance">
        <CodeGroup>
          ...
          // Create a Notion MCP server instance
          const notionServer = await klavis.mcpServer.createServerInstance({
        ...
        <Info>
          **Response Information**: The API returns:
          - `serverUrl`: The URL for connecting your MCP client to Notion
          - `instanceId`: Unique identifier for your server instance
          - `oauthUrl`: OAuth authorization URL for Notion authentication
        </Info>
      </Step>
