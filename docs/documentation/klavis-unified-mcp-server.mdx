...
graph TD
    ...
    subgraph Klavis Platform
        U["<br>Unified MCP Server<br>&nbsp;"]
        subgraph Individual MCP Servers
            S1["<br>GitHub MCP Server<br>&nbsp;"]
            S2["<br>Jira MCP Server<br>&nbsp;"]
            S3["<br>Slack MCP Server<br>&nbsp;"]
        end
    end

    A -->|Single Connection| U
    U -->|Manages Connections| S1
    U -->|Manages Connections| S2
    U -->|Manages Connections| S3
```
...
The Unified MCP Server automatically namespaces tools to avoid conflicts. For example, if both GitHub and Jira have a `search_issues` tool, they will be exposed as `github_search_issues` and `jira_search_issues`.
